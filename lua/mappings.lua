...
wk.register({
  ...
  -- Views
   ["<leader>v"] = {
      name = "[Views]",
     n = { "<cmd>NeoTreeShowToggle<cr>", "[NEOTREE] Toggle" },
     t = { "<cmd>ToggleTerm<cr>", "[TOGGLETERM] Open new terminal" },
    e = { "<cmd>:edit ~/AppData/Local/nvim/lua/plugins/init.lua<cr>", "[EDIT] init.lua" },
    },
    ["<leader>c"] = {
        name = "[Compiler]",
        j = {"<cmd>!javac %<cr>","[Compiler] java"}
    },
        -- Refactoring
    ["<leader>r"] = {
        name = "[Code refactor]",
    ...
    ["<leader>d"] = {
        ...
        w = { "<cmd>Telescope dap variables<cr>", "[TELESCOPE DAP] Wariables" },
         ...
         ["<c-l>"] = { "<cmd>noh<cr>", "Clean search highlights" },
},n_opts)
-- wk.register({
